...
Dans les fichiers annexes : On retrouve le Code de Conduite de l'utilisation du Git. Dans ce dernier, on voit qu'il faut rester dans un contexte approprié pour le travail, et rester respectueux envers les autres utilisateurs et développeurs du site web.

On apprend aussi qu'il ne faut que lancer les codes par pull requests, et que seule une personne admin peut accepter le code envoyé. 
...
On retrouve aussi des explications de code dans le wiki, même si pas très détaillées.
...
On compte 63 erreurs traitées dans Closed, avec aucune traitée depuis 12/2025

Dans pull requests : On remarque qu'il ne s'y trouve aucun request ouvert, ce qui veut dire que tous les pulls requests ont été acceptés.
...
Selon GitHub, il manque une Security Policy, une Issue Template (pourtant bien intégré), une Pull Request Template, et une Repository admin to accept content reports.
...
De base, il faut savoir que le GPU et le CPU ont besoin d'un langages différents, donc à traduire pour travailler ensemble.
Cela rend donc le produit très intéressant.

On a ensuite deux choix différents afin d'installer Hybridizer sur son ordinateur;

-premier choix : pour les personnes très expérimentées, sans tutoriel, juste avec les prérequis, le code pour installer et la vérification.

-deuxième choix : Passage par tutoriel avec une installation en 15 min: 

On ouvre avec une phrase d'accroche qui dit, qu'en partant de 0, on peut construire un code 
capable d'accélérer les performances de sa carte graphique. De plus, nous avons un tableau des softwares et du hardware prérequis pour l'installation de Hybridizer.

On a ensuite 6 grandes étapes d'installation;

- Installation de Driver NVIDIA : On trouve ici un lien vers le site de NVIDIA.
    Blocage : Sans essayer d'installer le driver, le site renvoie vers la page d'accueil de NVIDIA, sans qu'il soit précisé 
...
avec une petite console, mais on a pas de contexte supplémentaire.
    ...
    Solutions : Face à ces problèmes, une bonne solution serait d'ajouter plus de captures d'écran, pour que le nouvel utilisateur puisse comprendre sans trop réfléchir ce qu'il est censé faire.
...
- Installation Hybridizer : Si Hybridizer est vraiment disponible sous forme de package, alors cette étape semble plutôt facile.
...
À Vérifier la semaine prochaine, pendant la tentative d'installation :
...
- Explorer le site de NVIDIA pour vérifier si l'installation du driver est simple.
- Vérifier si l'installation de NET8.0 est simple.
- Troubleshooting, y'a t-il des failles non ramassées dans le tableau donné sur le site ?
...
Réponse : En anglais, il s'écrit Microsoft Intermediate Language. 
...
Cependant, il agit après le code et le traduit dans le langage du composant. Il a donc un rôle similaire à celui d'Hybridizer, mais est moins efficace.
...
ainsi que les langages sur lesquels Hybridizer marche. On retrouve notamment le OPENCL, le C++, le AVX, le CUDA etc...
...
- Possibilité d'appeler ces backends depuis le .NET
...
de la sortie du signal après être passé par le langage intermédiaire.
...
- Diversité; En écrivant un code, on peut l'exécuter sur plusieurs composants. 
...
entre autre, ce fichier Markdown. On pourra utiliser ce repository dans la suite du stage pour y déposer d'autres fichier.
...
Réponse : En anglais, le CPU s'appelle le Central Processing Unit, aussi appelé microprocesseur.

Physiquement, il s'agit d'une puce qui exécute une suite de tâches appelées programmes. Cette dernière est composée de milliers de transistors.
L'architecture sur laquelle cette puce repose s'appelle l'architecture Von Neumann. 

Si l'on décortique cette puce, elle est composée d'une unité de contrôle, et d'une unité de Logique Arithmétique.
Sur les CPU de base, comme ils étaient au début, on retrouve un Data Bus, un Address Bus, et un Control Bus.

Question : À quoi sert le Data Bus ?

Réponse : Le Data Bus sert à transférer les données de l'entrée à la sortie.

Question : À quoi sert l'Address Bus ?

Réponse : L'Address Bus sert à trouver l'endroit où il faut prendre ou envoyer les données.
...
Execute : La tâche est ensuite effectuée en fonction de la valeur traduite.

Tout CPU fonctionne avec un signal horloge, qui est mesuré en GHz, ce qui lui permet de calculer des milliers de fois par seconde.
...
Le compteur programme retient donc qu'à l'adresse 0000, il faut exécuter la valeur de 1010, et ainsi de suite. 
Chaque valeur binaire comprend sa propre instruction, comme dit auparavant.

Pour coder sur un CPU, il faut pouvoir lui donner un code qu'il comprends.
...
peuvent avoir plusieurs cores, qui peuvent chercher entre elles le cycle Fetch-Decode-Execute.
...
Pour faire simple, un GPU possède des milliers de coeurs plutôt simples. Cela permet au GPU de calculer 
...
Comme pour le CPU, le GPU possède une entité mémoire qui s'appelle la VRAM. 
...
il a besoin d'une mémoire différente de celle du CPU. 
...
de manière permanente. Contrairement à la RAM ou la VRAM, ces données restent présentes même lorsque l'ordinateur 
est éteint. 
...
On a vu tout à l'heure que certains CPU modernes pouvaient contenir un GPU intégré. Ceci est vrai, mais ce n'est pas l'architecture
la plus performante. Si le GPU se trouve dans la même puce que le CPU, alors il ne peut pas posséder sa propre VRAM, 
...
changements de données à partir d'une seule instruction. On peut donc voir le GPU comme une grande machine 
SIMD spécialisée.
...
utilisé pour coder directement sur les cartes graphiques NVIDIA. On appelle un CUDA core un coeur CUDA, 
qui agit comme un mini-processeur capable de faire des opérations mathématique simple.

On s'intéresse maintenant à l'ancien site : https://docs.altimesh.com/articles/CUDABasicConcepts.html.
...
L'idée est donc, au lieu d'avoir quelques gros threads sur le CPU, CUDA les transforme en pleins de petits threads qui traitent indépendamment
la requête demandée.

Principe fondamental de CUDA : Organisation par Grid, Block, Warp et Thread. 
...
Si j'ai bien compris, il y a une faute d'orthographe. Sinon cela veut dire que je n'ai pas compris ce qu'est un Block Run.

Problème dans les sites : on explique très brièvement threadIdx et blockIdx, mais jamais blockDim.

Dans l'ancien site, on continue ensuite de nous expliquer le fonctionnement en code CUDA, ce qui nous 
intéresse moins que ce qui se trouve sur le site docs Hybridizer.

En effet, on va maintenant retourner sur l'autre site, dans l'onglet Core Concepts.

Cet onglet décrit donc les concepts fondamentaux. Décortiquons la première phrase du site, qui paraît compliquée à la première lecture.
...
Définition Kernels : Fonctions conçues pour s'exécuter en parallèle sur un GPU. Ici, la phrase "s'exécuter en parallèle" 
se traduit pas le fait d'envoyer une tâche qui est faites plein de fois en simultané. Il s'agit donc d'un système en SIMD.

Définition Points d'entrée : Il s'agit de l'endroit où le GPU commence son execution du programme.
...
On appelle en OpenCL les blocks les Work Groups, et les threads les Work Items. 
...
La différence reste dans le fait qu'il faut réécrire son code existant pour le faire marcher, Alors qu'avec 
...
Lorsqu'on prend en entrée le thread 0 du block 0, on a threadIdx = blockIdx = 0.
...
Le but de ce code est plutôt facile, car il s'agit uniquement de mettre le tableau au carré le tableau a,
...
une méthode statique avec une implémentation interne. 
...
En fait, la structure du code fait exactement la même chose que le code d'avant, mais est cette fois-ci beaucoup plus facile à écrire.
Cependant, au lieu de mettre tous les éléments du tableau au carré, il rajoute 1 à toutes les cases. 
S'il faisait exactement la même chose que le tableau d'avant, le code aurait pris cette forme :
...
On nous explique que le code transformé a besoin de passer par plusieurs étapes, et que ces étapes seront expliquées dans cet onglet.

Remarque : Le Schéma est trop petit et n'est pas lisible, sauf en ouvrant l'image dans un autre onglet.

Première partie du schéma : Passage du User Source Code à l'assemblage .NET :

Lorsque l'on code en C#, ou en VB.NET, le code est automatiquement téléchargé en fichier de configuration. 
En effet, on appelle le code écrit par l'humain le code source, et la deuxième étape se charge de le transformer 
...
Dans cette partie, la configuration Hybridizer prend en compte tous les méthodes ou les types à 
...
prend sa "saveur" par rapport à ce qu'il a retenu dans sa configuration de base. 
...
composant adapté. Hybridizer traduit donc le même code pour chaque composant, et ressort donc un backend avec d'autres 
...
On a ensuite un exemple de l'étape 2 : les paramètres des constructeurs (Build Parameters) :

Il s'agit d'un code MSBuild (donc en XML) qui est déjà intégré dans le package NuGet. 
...
Remarque : à part le nom des méthodes et une très petite explication, on ne nous montre pas du tout comment les intégrer
...
Flavor CUDA, le code C est traduit en code nvcc, et produit internalement des fichiers en .cu/.cuh.
...
Réponse : C'est un concept assez simple, qui consiste à diviser le travail entre plusieurs coeurs CPU 
...
Remarque : On a l'impression que cette partie n'a pas été complétée, car on ne sait pas si il s'agit d'un package
...
(float, double, int, long) sont copiés facilement au GPU. La seule différence reste les tableaux de struct.

Question : Qu'est-ce-qu'un tableaux de struct ?
...
La phrase "marshalled as contiguous memory" veut dire que le tableau Vector3 est transféré par mémoire contiguë, 
...
depuis plusieurs années sur le GitHub. De plus, sur le GitHub, il y a des manques signalés par GitHub lui-même, 
...
Grâce aux questions que je me suis posé, sur les différentes abréviations ou les mots que je ne connaissait pas, 
